package org.shersfy.server.utils;
...
import org.shersfy.server.utils.HttpUtil.FormField.FormFieldType;
	...
	public static HttpResult send(String url, String method, List<NameValuePair> params, Map<String, String> headerMap){
		...
		try {
			...
			if(httpResult.getCode()==0){
				httpResult.setCode(httpResponse!=null?httpResponse.getStatusLine().getStatusCode():ResultCode.NG);
			}
			...
			if (formFields != null) {
				for (FormField field : formFields) {
					...
					case File:
						...
						inputStream.skip(field.getOffset());
